...
const request = { name: 'Test Customer', phone: '2015550100', email: 'test@example.com', zip: '07601', municipality: 'Hackensack', appliance: 'Washer', issue: 'Will not drain', date: '2026-10-01', sourcePage: '/new-jersey/bergen-county' };
...
test('routes CT and representative Bergen ZIPs without accepting all NJ ZIPs', () => {
  ...
  for (const zip of ['07030', '07102', '10001', '00000', '7601']) expect(branchForZip(zip)).toBeNull();
});
...
test('covers 70 municipalities with a focused set of ten unique town routes', () => {
  ...
  for (const town of BERGEN_TOWNS) {
    ...
    expect(bergenMetadata(town).alternates?.canonical).toBe(`https://www.myappliance.us/new-jersey/bergen-county/${town.slug}`);
  }
});
...
test('server resolves branch from location, ignoring a forged browser branch', () => {
  ...
  if (nj.ok) expect(nj.data).toMatchObject({ branchId: 'nj', branchName: 'Bergen County, NJ', phone: '+12015550100' });
...
test('server rejects malformed and out-of-area requests before dispatch', () => {
  for (const invalid of [null, [], {}, { ...request, phone: 'bad' }, { ...request, zip: '07030' }, { ...request, municipality: 'Hoboken' }, { ...request, issue: 123 }, { ...request, email: 'not-an-email' }]) {
...
test('email display escapes customer markup; source tracking excludes query data', () => {
  ...
  const result = validateBookingRequest({ ...request, sourcePage: '/booking?email=private@example.com' }, true);

  ...
  return (
    <div className="relative" ref={ref}>
      ...
      <button
        ...
        id={id}
        onClick={() => setOpen((o) => !o)}
        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border bg-white text-left transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer ${
          error
            ? 'border-red-400'
            : open
      ...
      {/* Calendar dropdown */}
      {open && (
        <div className="absolute z-50 mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl p-4 w-72">
          ...
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              onClick={prevMonth}
              disabled={!canGoPrev}
              className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer"
            ...
            {cells.map((day, i) => {
              ...
              return (
                <button
                  ...
                  disabled={isDisabled}
                  className={`w-8 h-8 mx-auto flex items-center justify-center rounded-lg text-sm font-medium transition-colors duration-150 cursor-pointer
                    ${isSelected
          ...
          <div className="flex items-center gap-3 mt-3 pt-3 border-t border-slate-100 text-xs text-slate-400">
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-[#ffb81c]/20 inline-block" /> Today
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-blue-950 inline-block" /> Selected
            </span>

...
function RepairCard({ repair, index }: { repair: typeof repairs[0]; index: number }) {
 ...
 <div>
 <span className={`inline-block text-xs font-bold px-2.5 py-1 mb-2 ${categoryColors[repair.category] ??'bg-blue-100 text-blue-700'}`}>
 {repair.category}
 </span>
 ...
 <span key={part} className="text-xs bg-slate-50 border border-slate-200 text-slate-600 px-2.5 py-1 font-medium">
 {part}
 </span>
))}
 </div>

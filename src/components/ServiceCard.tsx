        ...
        {activeTab === 'parts' && (
          <div>
            ...
            <div
              ...
              aria-label={`Common parts replaced for ${service.title}`}
            >
              {service.parts.map((part) => (
                <span
                  key={part}
                  className="text-xs bg-blue-50 border border-blue-100 text-blue-700 px-2.5 py-1 font-medium"
                >
                  {part}
                </span>
              ))}
      ...
      <div className="px-5 pb-4 pt-2 border-t border-blue-50">
        ...
        <div className="flex flex-wrap gap-1.5">
          {service.brands.map((brand) => (
            <span
              key={brand}
              className="text-xs bg-slate-50 border border-slate-100 text-slate-500 px-2 py-0.5 font-medium"
            >
              {brand}
            </span>
          ))}

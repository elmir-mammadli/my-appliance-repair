...
function DiscountCard({ item, index }: { item: typeof discounts[0]; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`relative bg-white/10 hover:bg-white/15 border border-white/20 hover:border-white/35 rounded-2xl p-6 flex flex-col gap-4 transition-all duration-500 group ${
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="w-14 h-14 bg-white/15 rounded-xl flex items-center justify-center text-white flex-shrink-0">
          {item.icon}
        </div>
        <span className={`${item.accentColor} text-sm font-extrabold px-3 py-1.5 rounded-lg tracking-wide flex-shrink-0`}>
          {item.badge}
        </span>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-blue-200 text-lg max-w-2xl mx-auto leading-relaxed">
            CT Appliance Repair is proud to offer special pricing for the people who keep our communities safe, educated, and protected. Discounts apply to labor on all repair services.
          </p>
        </div>
        ...
        {/* Bottom Banner */}
        <div className="bg-white/10 border border-white/20 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
